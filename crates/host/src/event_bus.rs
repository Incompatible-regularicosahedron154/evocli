...
impl EventBus {
    pub fn new() -> Self {
        ...
        tokio::spawn(async move {
            let db_path = dirs::home_dir()
                ...
                .join(".evocli")
                .join("events.db");
            let _ = std::fs::create_dir_all(db_path.parent().unwrap_or_else(|| std::path::Path::new(".")));
            
            let conn = match Connection::open(&db_path) {
                Ok(c) => c,
                Err(e) => { eprintln!("EventBus DB error: {}", e); return; }
            };
    ...
    pub fn tool_called(&self, session_id: &str, tool: &str, success: bool) {
        self.emit(session_id, "tool_call", serde_json::json!({
    ...
    pub fn skill_executed(&self, session_id: &str, skill_id: &str, step: &str) {
        self.emit(session_id, "skill_exec", serde_json::json!({
    ...
    pub fn memory_recalled(&self, session_id: &str, query: &str, hits: usize) {
        self.emit(session_id, "memory_recall", serde_json::json!({
