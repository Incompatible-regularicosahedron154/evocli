...
pub fn rrf_merge_with_config(
    ...
    let bm25_map: HashMap<&str, &Bm25Result> = bm25_results.iter()
        .map(|r| (r.symbol_id.as_str(), r)).collect();
    let vec_map: HashMap<&str, &VectorResult> = vector_results.iter()
        .map(|r| (r.symbol_id.as_str(), r)).collect();
    ...
    ranked.into_iter().map(|(id, rrf_score)| {
        let bm = bm25_map.get(id.as_str());
        let vr = vec_map.get(id.as_str());
        HybridResult {
            symbol_id:   id.clone(),
            name:        bm.map(|r| r.name.clone()).or_else(|| vr.map(|r| r.name.clone())).unwrap_or_default(),
            kind:        bm.map(|r| r.kind.clone()).unwrap_or_default(),
            file:        bm.map(|r| r.file.clone()).or_else(|| vr.map(|r| r.file.clone())).unwrap_or_default(),
