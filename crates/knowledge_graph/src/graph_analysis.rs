...
impl KnowledgeGraph {
    ...
    pub fn from_sqlite(db_path: &std::path::Path) -> Result<Self> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, name, kind, file, language FROM symbols"
        )?;
    ...
    ///   4. 合并同标签节点 → 社区
    pub fn detect_communities_with_params(&self, max_iter: usize, min_community_size: usize) -> Vec<Community> {
        ...
        let mut communities: Vec<Community> = label_to_members
            ...
            .map(|(i, (_, members))| {
                ...
                let internal_edges = self.graph.edge_references()
                    .filter(|e| member_set.contains(&e.source()) && member_set.contains(&e.target()))
                    .count() as f64;
                let cohesion = if n > 1.0 { internal_edges / (n * (n - 1.0)) } else { 1.0 };
    ...
    fn community_label(&self, members: &[NodeIndex]) -> String {
        ...
        file_counts.into_iter().max_by_key(|(_, c)| *c).map(|(k, _)| k).unwrap_or("misc".into())
    }
    ...
    fn merge_small_communities_threshold(&self, communities: Vec<Community>, min_size: usize) -> Vec<Community> {
    ...
    pub fn blast_radius(&self, symbol_id: &str, max_depth: usize) -> Option<BlastRadius> {
        ...
        while let Some((node, depth)) = queue.pop_front() {
            if depth > 0 {
                ...
                upstream.push(SymbolRef { id: s.id.clone(), name: s.name.clone(), file: s.file.clone(), depth });
        ...
        while let Some((node, depth)) = queue.pop_front() {
            if depth > 0 {
                let s = &self.graph[node];
                downstream.push(SymbolRef { id: s.id.clone(), name: s.name.clone(), file: s.file.clone(), depth });
                files.insert(s.file.clone());
            }
            if depth < max_depth {
    ...
    pub fn detect_processes(&self, max_depth: usize) -> Vec<ExecutionFlow> {
        let entry_patterns = ["main", "run", "handle", "execute", "start", "init", "new", "process"];
        let mut flows = Vec::new();
        let mut seen_entries: HashSet<NodeIndex> = HashSet::new();

        for node in self.graph.node_indices() {
            let sym = &self.graph[node];
            let is_entry = entry_patterns.iter().any(|p| sym.name.to_lowercase().contains(p));
            if !is_entry || seen_entries.contains(&node) { continue; }
            ...
            while let Some((cur, depth)) = stack.pop() {
                ...
                if depth < max_depth {
                    for edge in self.graph.edges_directed(cur, petgraph::Direction::Outgoing) {
                        let tgt = edge.target();
                        if !visited.contains(&tgt) {
                            visited.insert(tgt);
