...
use crate::{MemoryLabel, labeler::TrainingStore};
...
impl ModelTrainer {
    ...
    pub fn train(&self) -> Result<SerializedModel> {
        ...
        if samples.is_empty() { anyhow::bail!("No training samples"); }
        let n = samples.len();
        ...
        let correct = x.dot(&w.t()).rows().into_iter().zip(labels.iter())
            .filter(|(row, &lbl)| {
                ...
                row.iter().enumerate().max_by(|(_, a), (_, b)| {
                    a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal)
                })
                .map(|(i, _)| i).unwrap_or(0) == lbl
            }).count();
