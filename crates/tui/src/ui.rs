...
const USER_TEXT:    Color = Color::Rgb(202, 211, 245); // 用户消息正文

// AI accent — 绿色系（更柔和的薄荷绿，不刺眼）
const AI_ACCENT:    Color = Color::Rgb(166, 218, 149); // #a6da95 AI标识
const AI_TEXT:      Color = Color::Rgb(202, 211, 245); // AI正文
...
const C_BLUE:   Color = Color::Rgb(138, 173, 244); // #8aadf4 信息/链接 - 天空蓝
    ...
    let (status_text, status_color, spin) = match &app.state {
        ...
        AppState::Streaming { tokens_received } => {
            ...
            let s = if tok.is_empty() {
                format!("Streaming{elapsed_hint}")
            } else {
                format!("Streaming  {tok}{elapsed_hint}")
            };
