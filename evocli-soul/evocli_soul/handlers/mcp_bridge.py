...
Connection Pool (PROMPT-03, MCP Spec 2025):
Instead of spawning a new process per tool call (0.5-1s for Node.js),
we keep one persistent process per server config (keyed by hash).
Each server is serialized via asyncio.Lock to prevent message interleaving.
Pool is cleaned up on process exit via atexit.
"""
...
# ── Connection Pool ────────────────────────────────────────────────────────────
# Ref: MCP Specification Session Lifecycle (2025), mcp-mux architecture
...
class _PooledConnection:
    ...
    call_count: int = 0
    max_calls: int = 200   # recycle after N calls to prevent memory leaks


_connection_pool: dict[str, _PooledConnection] = {}
_pool_lock: asyncio.Lock | None = None   # initialized lazily (must be in event loop)


def _pool_key(program: str, args: list[str]) -> str:
    """Stable hash key for a server config."""
    return hashlib.md5(f"{program}:{args}".encode()).hexdigest()
...
async def _get_or_create_connection(program: str, args: list[str]) -> _PooledConnection:
    """Get an existing healthy connection or create a new one."""
    key = _pool_key(program, args)
    pool_lock = await _get_pool_lock()

    async with pool_lock:
        conn = _connection_pool.get(key)

        # Check if existing connection is still alive and within call limit
        if conn is not None:
            proc = conn.client._proc
            if proc is not None and proc.returncode is None and conn.call_count < conn.max_calls:
                return conn
            # Stale connection — close and recreate
            log.debug("MCP connection stale (returncode=%s, calls=%d) — reconnecting",
                      proc.returncode if proc else "None", conn.call_count if conn else 0)
            try:
        ...
        try:
            ...
            log.warning("MCP connect failed for %s %s: %s", program, args, e)
            raise
        conn = _PooledConnection(client=client)
        _connection_pool[key] = conn
        log.debug("MCP pool: new connection %s %s (key=%s)", program, args[:2], key[:8])
        return conn


def _cleanup_pool() -> None:
    """Terminate all pooled MCP server processes on exit."""
    for conn in list(_connection_pool.values()):
        try:
            proc = conn.client._proc
    ...
    async def _read_loop(self) -> None:
        while self._proc and self._proc.stdout:
            try:
                ...
                log.debug("MCP read_loop parse error: %s", e)
...
async def _load_server_tools(server_name: str, program: str, args: list[str]) -> list[dict]:
    """Connect to MCP server and fetch tool list using pooled connection."""
    try:
        conn = await _get_or_create_connection(program, args)
...
async def call_mcp_tool(tool_key: str, arguments: dict) -> Any:
    """Execute an MCP tool call using pooled persistent connection.
    
    Replaces the previous short-lived connection pattern (0.5-1s per call)
    with a persistent connection pool. First call per server incurs startup
    cost; subsequent calls are <50ms.
    
    Ref: MCP Specification Session Lifecycle (2025)
    """
    info = _mcp_tools.get(tool_key)
    if not info:
        raise ValueError(f"Unknown MCP tool: {tool_key}")

    conn = await _get_or_create_connection(info["program"], info["args"])
    async with conn.lock:   # Serialize access to prevent message interleaving
        conn.call_count += 1
        result = await conn.client.call_tool(info["name"], arguments)
    return result
